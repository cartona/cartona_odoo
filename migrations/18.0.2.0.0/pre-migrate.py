...
def _update_ir_model_fields(cr):
    for renames in FIELD_RENAMES.values():
    ...
    for columns in COLUMNS_TO_DROP.values():
...
def migrate(cr, version):
    if not _table_exists(cr, 'marketplace_config'):
        _logger.info('18.0.2.0.0 pre-migrate: marketplace schema already converted')
        return

    _logger.info('Running cartona_odoo 18.0.2.0.0 pre-migration (marketplace -> cartona)')
    ...
    _logger.info('cartona_odoo 18.0.2.0.0 pre-migration complete')
